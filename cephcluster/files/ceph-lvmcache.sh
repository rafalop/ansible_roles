#!/bin/bash                                                                    
## Deploy one or more Ceph OSDs with lvmcache
...
Optional parameters:                    
  ...
  --cache-mode  cache mode to use (only when using MQ policy, default ${CACHE_MODE})
  --smq  use SMQ policy (untunable) instead of MQ
  --seq-cutoff {string in BYTES}  (only when using MQ policy, default $SEQ_CUTOFF)
...
then
  ...
  cache_vg=`pvs | grep $cache_dev | awk '{print $2}'`
  if [[ "$cache_vg" == "" ]]
  then
    pvremove $cache_dev
  else
...
# Attach lvm cache
# get vg name for hdd osd
osd_vg=`pvs | grep $DATA_DEVICE | awk '{print $2}'`
# extend with cache partition
get_shortname $DATA_DEVICE
osd_cache_part=`blkid -o device -t PARTLABEL="${SHORTNAME}_cache"`
cmd="vgextend $osd_vg $osd_cache_part"
runcmd "$cmd"
# create lv on new extension
cmd="lvcreate $osd_vg -l99%free $osd_cache_part -n osd-cache-`uuidgen`"
runcmd "$cmd"
# convert to cache
osd_lv=`lvs | grep $osd_vg | grep osd-block | awk '{print $1}'`
osd_cache_lv=`lvs | grep $osd_vg | grep osd-cache | awk '{print $1}'`
cmd="lvconvert -y --type cache --cache-pool ${osd_vg}/${osd_cache_lv} --cachemode $CACHE_MODE ${osd_vg}/${osd_lv}"
runcmd "$cmd"
# tune cache
if [[ $SMQ -eq 1 ]]
then
  lvchange --cachepolicy smq ${osd_vg}/${osd_lv}
else
  lvchange --cachepolicy mq --cachesettings "sequential_threshold=${SEQ_CUTOFF} random_threshold=${SEQ_CUTOFF}" ${osd_vg}/${osd_lv}
fi
